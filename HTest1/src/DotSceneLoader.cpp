...
void DotSceneLoader::processLight(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	if(sValue == "point")
		...
		pLight->setSpotlightRange(Degree(30), Degree(50));
	}
	...
	if(pElement){
		Vector3 dir = parseVector3(pElement);
		dir.normalise();
...
Quaternion DotSceneLoader::parseQuaternion(TiXmlElement *XMLNode)
{
	...
	{
		orientation.x = StringConverter::parseReal(XMLNode->Attribute("x"));
		orientation.y = StringConverter::parseReal(XMLNode->Attribute("y"));
		orientation.z = StringConverter::parseReal(XMLNode->Attribute("z"));
		orientation.w = StringConverter::parseReal(XMLNode->Attribute("w"));
	}
